...
public class Kamerabewegung : MonoBehaviour {
	...
	void Update () {
		...
		if (on) {
			...
			if ((Input.GetAxisRaw ("Mouse ScrollWheel") == 0.1f || Input.GetKey ("r")) && transform.position.y >= minhigh && Input.GetKey("y") == false) {
				transform.Translate (0, -zoom, 0);
				cam.transform.Rotate (-zoom * moveturn, 0, 0);
			}
			if ((Input.GetAxisRaw ("Mouse ScrollWheel") == -0.1f || Input.GetKey ("f")) && transform.position.y <= starthohe && Input.GetKey("y") == false) {
				transform.Translate (0, zoom, 0);
				cam.transform.Rotate (zoom * moveturn, 0, 0);
			}
			if (Input.GetMouseButton(1)) {
				transform.Translate (Input.GetAxis("Mouse X")*mouseSpeed,0,Input.GetAxis("Mouse Y")*mouseSpeed);
